...
%%%%%%%%%%%%%%%%%%%%%% NICHING VARIANTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This example case changes the base vector (FM_pm3) of all strategies to the
% nearest neighbor vector of each individual in the population. As such all
% strategies change as follows:
%
% I_strategy	 1 --> DE/rand/1 *becomes:* DE/nrand/1
%                2 --> DE/local-to-best/1 *remains the same*
%                3 --> DE/best/1 with jitter *remains the same* 
%                4 --> DE/rand/1 with per-vector-dither *becomes:* DE/nrand/1 with per-vector-dither
%                5 --> DE/rand/1 with per-generation-dither *becomes:* DE/nrand/1 with per-generation-dither
%                6 --> DE/rand/1 either-or-algorithm *becomes:* DE/nrand/1 either-or-algorithm
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%-----provided D = 2 Shubert------------------------------------------------
if (I_plotting == 1)      
   FVc_xx = [-10:0.2:10]';
   FVc_yy = [-10:0.2:10]';

   [FVr_x,FVr_y]=meshgrid(FVc_xx',FVc_yy') ;
   FM_meshd = zeros(size(FVr_x));
   for i=1:length(FVc_xx)
	   for j=1:length(FVc_yy)
		   X = [FVr_x(i,j), FVr_y(i,j)];
		   FM_meshd(i,j) = shubert(X);
	   end
   end
...
[FVr_x,S_y,I_nf] = deoptniching('objfun',S_struct)
